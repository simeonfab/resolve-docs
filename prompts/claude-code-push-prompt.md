...
I need you to update one file in this repo and push it.

Repo: simeonfab/ResolveOS-intake-frontend (locate it locally, or clone it if not already present)
File to replace: public/index.html

Steps:
1. Find the current public/index.html in this repo.
2. Replace its entire contents with the new version below — full replacement, not a merge or partial edit.
3. Stage and commit the change: git add public/index.html && git commit -m "Fix screen 0 two-step reveal and sticky bottom bar"
4. Push to the main branch: git push origin main
5. Confirm the push succeeded and tell me the commit hash.

Do not modify anything else in the repo. Do not touch api/resolve.js, package.json, or any other file.

Here is the full replacement content for public/index.html:
...
.apt-title { font-size: 15px; font-weight: 600; color: white; line-height: 1.3; margin-bottom: 3px; }
.apt-sub { font-size: 12px; color: rgba(255,255,255,0.5); line-height: 1.4; }
...
<div class="phone">
  ...
  <div class="content">
    ...
    <div class="screen active" id="screen-0">
      <div id="name-step" class="screen0-step">
        <div class="eyebrow">Let's get started</div>
        <h1>What's your name?</h1>
        <div class="field" style="margin-top:20px;">
          <input type="text" id="name-input" placeholder="Your first name" onkeydown="if(event.key==='Enter'){event.preventDefault(); revealJourneyStep();}" />
        </div>
      </div>
      ...
      <div id="journey-step" class="screen0-step screen0-step-hidden">
        ...
        <div class="journey-option active" onclick="selectJourney(this)">
    ...
    <div class="screen" id="screen-1">
      ...
      <button class="collapse-toggle" onclick="toggleOptional()">＋ Add supporting material <span style="opacity:0.5;font-size:11px;">(optional)</span></button>
      ...
      <button class="collapse-toggle" onclick="loadExample()">＋ Load an example project (Nook &amp; Thread rebrand)</button>
    </div>
    ...
    <div class="screen" id="screen-2">
      ...
      <div class="confirm-row">
        ...
        <button class="btn-outline">↩️ Reframe</button>
      </div>
    ...
    <div class="screen" id="screen-3">
      ...
      <p class="subtitle" id="s3-sub">Based on what you shared.</p>
      ...
      <div class="divider"></div>
      <div class="section-title">Roadmap — built from your project</div>
    ...
    <div class="screen" id="screen-4">
      ...
      <div class="eyebrow" style="margin-top:20px;">What's next</div>
      <h2 id="s4-h">Where do you want to take this?</h2>
      <p class="subtitle" id="s4-sub">Your recommended first step is at the top.</p>
      <div class="action-card-primary">
        <div class="apt-icon">✅</div>
        <div>
          <div class="apt-title">Shortlist to one name and test it this week</div>
          <div class="apt-sub">Everything else — logo, website, packaging, comms — is blocked until this is done.</div>
        </div>
      </div>
...
function updateNav() {
  ...
  document.getElementById('progress-bar').style.width = progressPct[current] + '%';
  document.getElementById('progress-label').textContent = stepLabels[current];

  const sidebarMap = [0, 1, 1, 2, 3, 3];
  const activeSidebar = sidebarMap[current];
  for (let i = 0; i < 4; i++) {
...
function populateScreen4(d) {
  ...
  const subEl = document.querySelector('#screen-4 .apt-sub');
  if (titleEl) titleEl.textContent = d.recommendedAction || 'Your recommended next move';
  if (subEl) subEl.textContent = 'Resolve\'s recommended next move based on what you shared';
}
...
let screen0Step = 'name'; // 'name' | 'journey'
...
function revealJourneyStep() {
  ...
  setTimeout(() => {
    ...
    void journeyStep.offsetWidth; // force reflow so the transition fires
    document.getElementById('journey-greeting').textContent = `Hi ${n}, what do you need help with?`;
    document.getElementById('journey-h1').textContent = 'What do you need help with?';
    journeyStep.style.opacity = '1';
  }, 350);

  screen0Step = 'journey';
  document.getElementById('next-btn').textContent = 'Get started →';
  document.getElementById('s4-win-title').textContent = `You just cleared the fog, ${n}.`;
}

function selectJourney(el) {
...
function toggleOptional() {
  ...
  const btn = document.querySelectorAll('.collapse-toggle')[0];
  if (s.classList.contains('collapsed')) {
    s.classList.remove('collapsed');
    btn.textContent = '－ Hide supporting material';
  } else {
    s.classList.add('collapsed');
    btn.innerHTML = '＋ Add supporting material <span style="opacity:0.5;font-size:11px;">(optional)</span>';
  }
}
