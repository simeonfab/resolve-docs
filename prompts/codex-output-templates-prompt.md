...
api/resolve.js - both pasted in full at the bottom. Also a new reference file to add.

STANDING PRINCIPLES: modular, extensible code. Do not touch anything not listed below.
Ask rather than guess if context is missing. These changes are interconnected - read
the full prompt before starting.

================================================================================
OVERVIEW
================================================================================

This prompt implements the "Project Action Pack" output system for Resolve Intake.
The final screen (Screen 4) currently has one generic output action ("Output my
project plan") and five mostly-static secondary cards. This replaces that with a
fully dynamic, intelligent output system where every card is specific to the user's
actual project, selected from a master template library, and generates real usable
content when clicked.

Three connected pieces of work:
1. Tool detection on Call 1 + tool confirmation UI on Screen 2
2. Dynamic template selection on Call 2
3. Screen 4 rebuilt as an intelligent output hub

================================================================================
PART 1 - Tool detection (Call 1) + confirmation UI (Screen 2)
================================================================================

CALL 1 SCHEMA CHANGE (api/resolve.js - runUnderstandingCall):

Add one new field to the returned JSON schema:

  "mentionedTools": ["array of tool names detected in the raw project input"]

Examples of what to detect: Notion, GitHub, Cursor, Codex, Claude, ChatGPT,
Jira, Linear, Figma, Slack, Trello, Asana, Vercel, Supabase, Zapier, n8n,
Google Docs, Google Sheets, Airtable, Coda. Case-insensitive detection.

Rules:
- Only include tools explicitly mentioned in the user's input text
- Do not infer or guess tools not mentioned
- Empty array [] if nothing detected - this is valid and expected
- Maximum 5 tools (cap to most prominent if more detected)

Update validateUnderstandingShape() to include mentionedTools as a required field
(must be an array, can be empty, strings must be non-empty if present).

SCREEN 2 UI CHANGE (public/index.html):

After the four understanding cards and the Edit button, add a conditional tool
confirmation section. This section ONLY renders if mentionedTools is non-empty.

Visual treatment: a small section below the existing cards, titled something like
"Tools we noticed you mentioned" with a subtle section label. Show each detected
tool as a pill/chip that is tappable (toggles active/inactive - active by default).
At the end of the chip row, add a "＋ Add" chip. When tapped, that specific chip
smoothly transforms into a small inline text input (the chip itself expands - no
modal, no new element appearing elsewhere). User types a tool name, presses Enter
or taps a confirm icon, and it becomes a new chip in the row. The "＋ Add" chip
reappears after.

Store the final confirmed tools array in session state as confirmedTools[].
Pass confirmedTools through to Call 2 and Call 4.

If mentionedTools is empty, this entire section is invisible - no heading, no
empty state, nothing.

================================================================================
PART 2 - Dynamic template selection (Call 2)
================================================================================

CALL 2 SCHEMA CHANGE (api/resolve.js - runRecommendationCall):

Replace the existing "secondaryActions" array with a new "outputTemplates" array.

outputTemplates should contain exactly 6 objects, each representing one output
the user can generate from their project. Each object:

  {
    "templateId": "one of the 15 template IDs from the reference file",
    "label": "specific, project-relevant label (not generic)",
    "description": "one sentence explaining why this is relevant for this project",
    "icon": "single relevant emoji"
  }

The 15 template IDs and their selection rules are defined in the reference file
being added to the repo (resolve-output-templates.md). Call 2's system prompt
must include the full content of that file so it can make intelligent selections.

Key selection rules (also in the reference file):
- ai-continuation (Template 03) almost always included
- plain-english (Template 04) or share-message (Template 05) almost always included
- project-brief (Template 06) almost always included
- notion-brief (Template 09), ai-handoff (Template 10), github-issue (Template 15)
  only if those tools appear in confirmedTools[]
- name-ideas (Template 13) only if no project name was detected in the input
- risk-summary (Template 08) only if genuine risks are identifiable
- Never surface a template whose content would have to be invented to fill it
- If in doubt, pick the template whose output the user can act on most immediately

Also add to Call 2 schema:
  "topCardType": "action" | "decision"
  This tells the frontend whether to render the primary recommendation card as
  "Do this first" (action) or "Decide this first" (decision), based on whether
  the highest-leverage next move is an action or a decision that must be made first.

Update validateRecommendationShape() to validate outputTemplates (array of exactly
6 objects, each with non-empty templateId, label, description, icon strings) and
topCardType (must be "action" or "decision"). Remove secondaryActions from schema
and validation entirely.

================================================================================
PART 3 - Screen 4 rebuilt as intelligent output hub
================================================================================

SCREEN 4 FRONTEND CHANGES (public/index.html):

Primary recommendation card:
- Currently renders as "Do this first" always
- Change to render dynamically based on topCardType from Call 2:
  - If "action": header label reads "Do this first", icon stays as-is
  - If "decision": header label reads "Decide this first", use a different icon
    (e.g. ⚖️ or 🔀) to visually distinguish it from an action card

Output template grid:
- Remove the current hardcoded secondary action cards entirely
- Replace with a dynamic grid rendered from outputTemplates[] returned by Call 2
- Each card shows: icon, label, description (one line, smaller text)
- Clicking a card triggers generateOutput(templateId) - see below

generateOutput(templateId) function:
This is the core new frontend function. It takes a templateId and generates the
appropriate output content client-side using the existing session data (no new
API call for most templates). For each templateId:

- "key-decision": pull from Call 2's openDecision field if it exists, or
  synthesise from recommendedAction + why. Format as: decision statement,
  why it's blocking, 2-3 options with trade-offs.

- "one-week-plan": generate from Call 2's topPriorities and recommendedAction.
  Format as 3 specific actions with done-when conditions.

- "ai-continuation": assemble from full session state (project understanding +
  recommendation + gap answers). Format as a ready-to-paste AI prompt.

- "plain-english": generate from Call 1's project/goal/state fields.
  One paragraph, jargon-free, written for someone with no prior knowledge.

- "share-message": shorter version of plain-english. 2-3 casual sentences.

- "project-brief": assemble the full 9-section report content already generated
  by Call 4 (if available) or synthesise from Call 1+2 output. Format as
  structured markdown.

- "stakeholder-update": generate from Call 1+2 output. 3-4 short paragraphs
  in update format.

- "risk-summary": pull from Call 4's risks section if present, or generate
  from Call 1's uncertainty field and Call 2's doNotYet.

- "notion-brief": same as project-brief but formatted with Notion-style
  heading hierarchy and a property block at the top.

- "ai-handoff": different from ai-continuation - generates a context document
  for starting a fresh AI conversation, not continuing the current one.

- "stop-list": pull from Call 2's doNotYet field. Format as a clear list
  with one-line reasons for each item.

- "speculative-roadmap": pull from Call 4's speculative roadmap section.
  Clearly prefaced as extrapolation.

- "elevator-pitch": generate from Call 1's project/goal fields. 2 sentences
  max. Only generate if clear enough direction exists.

- "name-ideas": generate 5 name directions with one-line explanations. Only
  appears when surfaced by Call 2 (no name detected).

- "github-issue": generate from recommendedAction + milestone. Formats as
  a GitHub issue with title, description, acceptance criteria, label suggestions.
  Only appears when GitHub is in confirmedTools[].

OUTPUT DELIVERY PATTERN (same for all templates):
When the user clicks a card:
1. Card shows a small loading indicator (spinner or pulsing dots inside the card)
2. generateOutput() runs (client-side, near-instant for most templates)
3. Card transforms to show: the generated content in a collapsed preview
   (first 2 lines visible), plus two action buttons: "Copy" and "Close"
4. Copy button copies full content to clipboard, briefly shows "Copied ✓"
5. Close collapses back to the original card state

For "Export to a tool" type outputs (notion-brief, github-issue, ai-handoff):
if confirmedTools[] contains the relevant tool, the card label is already
specific (e.g. "Copy as Notion brief" not "Export to a tool"). No additional
expansion or dropdown needed - clicking directly generates and shows the output.

"Output my project plan" card:
Keep this as a separate distinct card (outside the 6-template grid), styled
differently (coloured accent as previously implemented). Its existing
handleProjectPlanClick() behaviour is unchanged - it still triggers Call 4
and generates the full downloadable report. This is the "get everything" option,
separate from the quick-copy template cards.

Free text input:
Keep the existing "Or ask Resolve something specific" textarea. This is the
escape valve for anything not covered by the 6 templates.

================================================================================
REFERENCE FILE TO ADD TO REPO
================================================================================

Add this file to the repo at: /resolve-output-templates.md
Full content provided at the bottom of this prompt.
This file must also be injected into Call 2's system prompt (alongside the
existing ResolveOS role/governance files already being injected) so the model
can make intelligent template selections.

================================================================================
OUT OF SCOPE FOR THIS PROMPT
================================================================================

- Do not implement voice transcription changes
- Do not change the processing reveal animations
- Do not change Screen 0, Screen 1, or Screen 3
- Do not add persistence, accounts, or saving
- Do not implement actual Notion/GitHub OAuth integrations - copy-to-clipboard
  is sufficient for all export destinations in this version

================================================================================
After implementing: git add -A && git commit -m "Implement dynamic output template
system: tool detection, template selection, Screen 4 output hub" && git push origin
main. Confirm commit hash. List any assumptions made. Flag if Call 2's prompt size
becomes a concern after adding the templates reference file injection.
================================================================================
...
NEW FILE TO ADD: /resolve-output-templates.md
--------------------------------------------------------------------------------
``` 
